...
$allowed_sort_columns = ['id', 'name', 'phone', 'address', 'item_code'];
...
try {
    if ($id) {
        $stmt = $pdo->prepare("UPDATE suppliers SET name = ?, phone = ?, address = ?, item_code = ? WHERE id = ?");
        $stmt->execute([$name, $phone, $address, $item_code, $id]);
        respond(true, [], 'Supplier updated successfully');
    } else {
        $stmt = $pdo->prepare("INSERT INTO suppliers (name, phone, address, item_code) VALUES (?, ?, ?, ?)");
        $stmt->execute([$name, $phone, $address, $item_code]);
